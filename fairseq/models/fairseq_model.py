...
class FairseqCTCEncoderDecoderModel(BaseFairseqModel):
    """Base class for encoder-decoder models.
    ...
    def __init__(self, encoder, ctc_encoder, decoder):
        ...
        self.decoder = decoder
        assert isinstance(self.encoder, FairseqEncoder)
        assert isinstance(self.ctc_encoder, FairseqEncoder)
        assert isinstance(self.decoder, FairseqDecoder)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, **kwargs):
        ...
        """
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, **kwargs)
        ctc_decoder_out = self.ctc_encoder(encoder_out, src_tokens)
        cmlm_decoder_out = self.decoder(prev_output_tokens, encoder_out=encoder_out, **kwargs)
        # print("Right here!!!")
        # print(src_tokens.shape, ctc_decoder_out.shape, cmlm_decoder_out[0].shape)
        return ctc_decoder_out, cmlm_decoder_out[0]
