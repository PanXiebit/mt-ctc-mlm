...
from . import (
    ...
    register_model, register_model_architecture, FairseqCTCEncoderDecoderModel,
...
@register_model('bert_transformer_seq2seq_ctc')
class Transformer_ctc_nonautoregressive(FairseqCTCEncoderDecoderModel):
    def __init__(self, encoder, ctc_encoder, decoder):
        super().__init__(encoder, ctc_encoder, decoder)
    ...
    def add_args(parser):
        ...
        parser.add_argument('--ctc-encoder-layers', type=int, metavar='N',
                            help='num ctc encoder layers')
    ...
    def build_model(cls, args, task):
        ...
        ctc_encoder = CTCTransformerEncoder(args, src_dict, encoder_embed_tokens, args.encoder_embed_scale)
        decoder = SelfTransformerDecoder(args, tgt_dict, decoder_embed_tokens, args.decoder_embed_scale)
        return Transformer_ctc_nonautoregressive(encoder, ctc_encoder, decoder)
...
class CTCTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_scale=None, left_pad=False):
        ...
        self.ctc_embed_positions = PositionalEmbedding(
            2 * args.max_source_positions, embed_dim, self.padding_idx,
            #left_pad=left_pad,
            #learned=args.encoder_learned_pos,
        ) if not args.no_ctc_token_positional_embeddings else None
        ...
        self.layers1.extend([
            ...
            for i in range(args.ctc_encoder_layers)
        ])
    ...
    def forward(self, encoder_out, src_tokens):
        """
        Args:
...
#             print(src_tokens.shape, x.shape, encoder_padding_mask.shape)
        for layer in self.layers1:
            x1 = layer(x, encoder_padding_mask)
            
        for layer in self.layers2:
            x2 = layer(x, encoder_padding_mask)
        
        x = torch.cat([x1, x2], dim=-1).transpose(0, 1).contiguous()  # B x T x 2C
        x = x.view(bs, 2 * src_len, embed_dim).transpose(0, 1) #  2T x B x C
        
        if self.normalize:
            x = self.layer_norm1(x)
            
        if self.ctc_embed_positions is not None:
            print("Using encoding position embedding!!!")
            ctc_src_tokens = torch.cat([src_tokens, src_tokens], dim=1)  # [B, 2T]  这里是有问题的。。，虽然暂时用不到，记得修改
            x = x + self.ctc_embed_positions(ctc_src_tokens)
        x = F.linear(x, self.embed_tokens.weight)  # 2T x B x vocab_size
        #encoder_padding_mask = torch.cat([encoder_padding_mask, encoder_padding_mask], dim=1)  # [B, 2T]
        return x
...
@register_model_architecture('bert_transformer_seq2seq_ctc', 'bert_transformer_seq2seq_ctc')
def base_architecture(args):
    ...
    args.ctc_encoder_layers = getattr(args, 'ctc_encoder_layers', 1)
    ...
    args.no_ctc_token_positional_embeddings = getattr(args, 'no_ctc_token_positional_embeddings', True)
...
@register_model_architecture('bert_transformer_seq2seq_ctc', 'bert_transformer_seq2seq_ctc_big')
