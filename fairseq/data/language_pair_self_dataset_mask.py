...
def collate(
    ...
    return {
        ...
        'total_ntokens': sum(s['total_ntokens'] for s in samples),
        'net_input': {
        ...
        'real_target': merge('real_target', is_target_list),
...
class LanguagePairSelfDatasetMask(FairseqDataset):
    ...
    def __getitem__(self, index):
        enc_source, dec_source, dec_target, real_target, ntokens, total_ntokens = self._make_source_target(self.src[index], self.tgt[index])
        return {'id': index, 'enc_source': enc_source, 'dec_source': dec_source, 'dec_target': dec_target,
                'real_target': real_target, 'ntokens': ntokens, 'total_ntokens': total_ntokens}
    ...
    def _make_source_target(self, source, target):
        ...
        total_ntokens = real_target.ne(self.tgt_dict.pad()).sum(-1).item()
        ...
        return enc_source, dec_source, dec_target, real_target, ntokens, total_ntokens
