...
@register_strategy('ctc_mask_predict')
class CTCMaskPredict(DecodingStrategy):
    
    def __init__(self, args):
        super().__init__()
        self.iterations = args.decoding_iterations
    
    def generate(self, model, encoder_out, src_tokens, src_lengths, tgt_dict):
...
#         seq_lens = seq_len - pad_mask.sum(dim=1)
        ...
        tgt_tokens, token_probs = self.generate_ctc(model, encoder_out, src_tokens, src_lengths)
    ...
    def generate_ctc(self, model, encoder_out, src_tokens, src_lengths):
        ctc_decoder_out = model.ctc_encoder(encoder_out, src_tokens)
        ctc_logits = ctc_decoder_out.log_softmax(-1).cpu().numpy()
        ctc_index = torch.argmax(ctc_decoder_out.log_softmax(-1), dim=-1)
        print(ctc_index)
        
        src_lengths = src_lengths.cpu().numpy()
        src_lengths = tf.cast(tf.convert_to_tensor(src_lengths), tf.int32)
        
        decoded, log_probabilities = tf.nn.ctc_beam_search_decoder(
            tf.convert_to_tensor(ctc_logits), tf.convert_to_tensor(src_lengths), 
            beam_width=100, top_paths=1)
        print(src_tokens.shape, ctc_logits.shape, decoded[0].values, decoded[0].indices)
